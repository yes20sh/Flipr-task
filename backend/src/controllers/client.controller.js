...
/* ---------------------------------------------------------- *
 * Helper: data‑URL → { buffer, contentType }
 * ---------------------------------------------------------- */
...
/* ---------------------------------------------------------- *
 * GET /clients  – list (returns imageUrl as data‑URL)
 * ---------------------------------------------------------- */
export const getAllClients = async (_req, res) => {
  try {
    const docs = await Client.find().lean();   // image buffer may come as BSON Binary
    ...
    const clients = docs.map(({ image, ...rest }) => {
      ...
      if (image?.data && image?.contentType) {
        const buffer = Buffer.isBuffer(image.data)
          ? image.data
          : Buffer.from(image.data?.buffer || image.data);  // handles BSON Binary
...
/* ---------------------------------------------------------- *
 * POST /clients – create (expects base‑64 in req.body.image)
 * ---------------------------------------------------------- */
...
/* ---------------------------------------------------------- *
 * PUT /clients/:id – update (image optional)
 * ---------------------------------------------------------- */
